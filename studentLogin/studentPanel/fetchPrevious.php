...
session_start();
	...
	{    
		...
		$currentIndexToDisplay=$_SESSION['prevIndexId'];	//variable to hold index of current question number to be displayed 
		$totalQuestions=$_SESSION['totalQuestions'];
		
		
		$questionIdToFetch=$arrayQuestionsIds[$_SESSION['prevIndexId']-1];
		$_SESSION['nextQuesIdTosave']=$questionIdToFetch;

		$_SESSION['prevIndexId']=$_SESSION['prevIndexId']-1;		//variable to hold index of previous question that can be selected from the questionIds array
	...
	if (($result4->num_rows)>0)	//if the student has already answered and saved this question
	{
		...
		echo json_encode(array('q' => $question, 'a' => $optionA, 'b' => $optionB, 'c' => $optionC, 'd' => $optionD, 'e' => $optionE, 'k' => $currentIndexToDisplay, 'f' => $totalQuestions, 'g' => $optionAnsweredQues, 'h' => $remainingTimeToReturn, 'j' => $logoutStatus, 'm' => $testStatus));
	}
?>
