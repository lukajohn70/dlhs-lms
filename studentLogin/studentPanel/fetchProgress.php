...
session_start();
	...
	if (($result4->num_rows)>0)	//if the student has already answered and saved this question
	{
		...
		echo json_encode(array('q' => $question, 'a' => $optionA, 'b' => $optionB, 'c' => $optionC, 'd' => $optionD, 'e' => $optionE, 'k' => $currentIndexToDisplay, 'f' => $totalQuestions, 'g' => $optionAnsweredQues, 'h' => $remainingTimeToReturn, 'i' => $greenFlag, 'j' => $logoutStatus, 'm' => $testStatus));
	}
?>
