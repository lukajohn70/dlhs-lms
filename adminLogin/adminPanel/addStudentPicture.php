...
session_start();
	...
	else
	{
		...
		{    
			$studentId = mysqli_real_escape_string($connection, $_POST['setStudentId']);
			...
			if(move_uploaded_file($_FILES['file1']['tmp_name'],$location))
			{
				...
				if($result1)
				{	
					header("location:addStudentForm.php?msgStatus1=1");
				}
				else
				{
					header("location:addStudentForm.php?msgStatus1=2");
				}
			}
		}
		else
