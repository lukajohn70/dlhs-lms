...
session_start();
	...
	else
	{
		...
		if(isset($_POST['updateStaffPicture3']))
		{    
			$staffId = mysqli_real_escape_string($connection, $_POST['setStaffId3']);
			...
			if(move_uploaded_file($_FILES['file3']['tmp_name'],$location))
			{
				...
				$query = "SELECT * FROM stafflogin WHERE staffId='$staffId'";
				$result = $connection->query($query);
				if (!$result) die($connection->error);
				$row = $result->fetch_array(MYSQLI_NUM);
				$passportPath='staffPassports/'.$row[8];
				unlink($passportPath);
				...
				if($result1)
				{						
					header("location:addStaffForm.php?msgStatus2=1");
				}
				else
				{
					header("location:addStaffForm.php?msgStatus2=2");
				}
			}
		}
		else
