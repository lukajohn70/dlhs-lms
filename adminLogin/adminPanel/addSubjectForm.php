  ...
  <body>
	...
	<section id="container" class="">
      ...
      <section id="main-content">
		  ...
		  <div class="row">
				<div class="col-lg-12">
					<h3 class="page-header"><i class="fa fa-file-text-o"></i> Add new Subject</h3>
					<ol class="breadcrumb">
							...
							<div class="panel-body">
								<form id="subjectForm">
									...
									<div class="form-group">
										<label>Name of Subject (e.g. <em>Mathematics</em>)</label>
										<input type="text" name="subjectName" class="form-control" placeholder="Enter subject name (e.g. Mathematics)" required >
									</div>
									<button type="button" class="btn btn-primary" onclick="addSubject()"><i class="fa fa-sign-in"></i> Submit</button>
								</form>
							</div>
						</section>
					</div>
					<div class="col-lg-12" id="tableColumn">
						<section class="panel">
							<header class="panel-heading" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px;">
								<span>View | Edit | Delete Subject</span>
								<button class="btn btn-success btn-sm" id="toggleFormBtn" onclick="dlhsToggleCreateForm()" style="font-weight: 700; border-radius: 6px; padding: 6px 12px;"><i class="fa fa-plus"></i> Add New Subject</button>
							</header>
							<div class="panel-body">
	  ...
	  <div id="myModal1" class="modal1"> <!--Start of Edit modal-->
				<div class="modal-content">
					...
					<div style="padding:15px; color:#fff; margin:10px 10px 0px 10px; background-color:#009999;"><center style="font-size:22px;">Edit Subject</center></div>
                    <div style="background-color:#E9F1EA; padding:15px; color:#fff; margin:0px 10px 0px 10px;">
                        <form method="post" id="editSubjectForm">
                            ...
                            <div class="item form-group">
								<div class="col-md-12">
									<label><span style="color:#000000;">Subject name to edit: <b><font id="subjectNameToEdit"></font></b></span></label><br>
									<label><span style="color:#000000;">Change subject name below: </span> <span class="required" style="color:red;">*</span></label><br>
									<input type="hidden" name="subjectId" required="required" id="subjectId">
									<input type="text" name="subjectName1" required="required" id="subjectName1" class="form-control">
								</div>
							</div>
							<div class="item form-group">
								<div class="col-md-12">
									<button class="btn btn-primary" type="reset">Reset</button>
									<button type="button" class="btn btn-success" onclick="editSubjectName()">Update</button>
								</div>
							</div>
                        </form>
                    </div>
	...
	<script>
		function dlhsToggleCreateForm() {
			...
			if (formCol.style.display === 'none') {
				...
				btn.innerHTML = '<i class="fa fa-plus"></i> Add New Subject';
					...
					success: function(response)
					{
						...
						for(var i=0; i<len; i++){
							...
							var tr_str = "<tr>" +
								"<td>" + (i+1) + "</td>" +
								"<td>" + subjectName + "</td>" +
								"<td align='center'><a onClick='openEditModal(\""+subjectId+"\",\""+subjectName+"\")' title='Edit this subject' style='cursor:pointer'><i class='fa fa-pencil-square-o'></i></a>&nbsp; &nbsp; &nbsp; &nbsp;<a onClick='deleteSubject(\""+subjectId+"\",\""+subjectName+"\")' style='pointer-events: none;' title='Delete this subject' style='cursor:pointer;'><i class='fa fa-trash' aria-hidden='true' style='color:red;'></i></a></td>" +
		...
		function deleteSubject(subjectId, subjectName)
		{
			...
			var toConfirm=confirm("Are you sure you wish to delete this subject?");
			if (toConfirm==true)
			{
					$.ajax({
						...
						success: function (html) {             
							...
							{                              
								 $('.message2').html("");
								 $('.message1').html('<i class="fa fa-check"></i> ' + subjectName + ' subject successfully deleted.').fadeIn('slow');
							...
							{                              
								 $('.message1').html("");
								 $('.message2').html('<i class="fa fa-times"></i> Could not delete subject. Please try again.').fadeIn('slow');
							}
						}
		...
		function editSubjectName()
		{
			...
			var subjectName = $('input[name=subjectName1]').val();
			
			if (subjectName=="")
			{
				$('.message3').html('<i class="fa fa-info-circle"></i> Please enter name of subject')
			}
			else
			{
				var form_data = 
				...
				$.ajax({
					...
					success: function (html) {             								
						...
						{                              
							$('.message3').html("");
							$('.message1').html('<i class="fa fa-check"></i> successfully edited to <b>' + subjectName + '</b>').fadeIn('slow');
						...
						{                              
							$('.message4').html("");
							$('.message3').html('<i class="fa fa-times"></i> Could not edit subject. Please try again.').fadeIn('slow');
						}
						else if (html==3)
						{                              
							$('.message4').html("");
							$('.message3').html('<i class="fa fa-times"></i> The subject <b>' + subjectName + '</b> already exists. Please enter another subject name.').fadeIn('slow');
						}
					}
				});
			}
		...
		function openEditModal(subjectId, subjectName)
        {
			...
			document.getElementById("subjectName1").value=subjectName;  
            $("#subjectNameToEdit").html(subjectName);  
